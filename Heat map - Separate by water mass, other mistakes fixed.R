...
for (i in 1:length(reps)){
  ...
  assign(paste(replicate, ".OTUnames", sep=""), OTUnames)
...
for (i in 1:length(reps)){
  ...
  OTUnames <- get(paste(replicate, ".OTUnames", sep="")) #Now uses the one relevant for that sample, instead of a "generic" OTUnames (this was never generic, it was M4)
